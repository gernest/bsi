...
func TestRow_Merge(t *testing.T) {
	tests := []struct {
		...
		{
			r1:  NewRow(1, 2, 3, shardwidth.ShardWidth+1, 2*shardwidth.ShardWidth),
			r2:  NewRow(3, 4, 5),
			exp: 7,
		},
		{
			r1:  NewRow(),
			r2:  NewRow(2, 66000, 70000, 70001, 70002, 70003, 70004),
			exp: 7,
		},
	}
...
func TestRow_Xor(t *testing.T) {
	r1 := NewRow(0, 1, shardwidth.ShardWidth)
	r2 := NewRow(0, 2*shardwidth.ShardWidth)
	exp := []uint64{1, shardwidth.ShardWidth, 2 * shardwidth.ShardWidth}
...
func TestRow_Union_Segment(t *testing.T) {
	r1 := NewRow(0, 1, shardwidth.ShardWidth)
	r2 := NewRow(0, 2*shardwidth.ShardWidth)
	exp := []uint64{0, 1, shardwidth.ShardWidth, 2 * shardwidth.ShardWidth}
	res := r1.Union(r2)
...
func TestRow_Difference_Segment(t *testing.T) {
	r1 := NewRow(0, 1, shardwidth.ShardWidth)
	r2 := NewRow(0, 2*shardwidth.ShardWidth)
	exp := []uint64{1, shardwidth.ShardWidth}
...
func TestRow_IsEmpty(t *testing.T) {
	r1 := NewRow(1, shardwidth.ShardWidth)
	r2 := NewRow(0, 2*shardwidth.ShardWidth)
	res := r2.Intersect(r1)
...
func TestRow_Includes(t *testing.T) {
	row := NewRow(0, 2*shardwidth.ShardWidth)
	...
	if !row.Includes(2 * shardwidth.ShardWidth) {
		t.Fatalf("row should include %d", 2*shardwidth.ShardWidth)
	}
}
