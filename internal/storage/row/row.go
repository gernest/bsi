...
func NewRowFromBitmap(b *roaring.Bitmap) *Row {
	...
	rowNum := uint64(0)
	for col, ok := b.MinAt(rowNum * shardwidth.ShardWidth); ok; col, ok = b.MinAt(rowNum * shardwidth.ShardWidth) {
		rowNum = col / shardwidth.ShardWidth
		seg := RowSegment{
			shard:    rowNum,
			data:     b.OffsetRange(rowNum*shardwidth.ShardWidth, rowNum*shardwidth.ShardWidth, (rowNum+1)*shardwidth.ShardWidth),
			writable: true,
		}
...
func NewRowFromRoaring(data []byte) *Row {
	bitmaps, shards := roaring.RoaringToBitmaps(data, shardwidth.ShardWidth)
...
func (r *Row) SetBit(i uint64) (changed bool) {
	return r.createSegmentIfNotExists(i / shardwidth.ShardWidth).SetBit(i)
}
...
// Columns returns a list of all columns set in the segment, normalized from 0-shardwidth.shardwidth-1
func (s *RowSegment) ShardColumns() []int64 {
	a := make([]int64, 0, s.Count())
	itr := s.data.Iterator()
	mask := uint64(shardwidth.ShardWidth - 1)
	for v, eof := itr.Next(); !eof; v, eof = itr.Next() {
