...
func Max(tx OffsetRanger, filter *roaring.Bitmap, shard, bitDepth uint64) (max int64, count uint64, err error) {
...
// A bitmap can be passed in to optionally filter the computed columns.
func Min(tx OffsetRanger, filter *roaring.Bitmap, shard, bitDepth uint64) (min int64, count uint64, err error) {
	consider, err := Row(tx, shard, bsiExistsBit)
	if err != nil {
...
func minUnsigned(tx OffsetRanger, filter *roaring.Bitmap, shard, bitDepth uint64) (min int64, count uint64, err error) {
	count = filter.Count()
...
func maxUnsigned(tx OffsetRanger, filter *roaring.Bitmap, shard, bitDepth uint64) (max int64, count uint64, err error) {
	count = filter.Count()
	for i := int(bitDepth - 1); i >= 0; i-- {
		row, err := Row(tx, shard, uint64(bsiOffsetBit+i))
		if err != nil {
