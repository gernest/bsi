...
func TestCursor_FirstNext(t *testing.T) {
	...
	if err := tx.CreateBitmap(rbf.Key{Shard: 1}); err != nil {
		t.Fatal(err)
	} else if _, err := tx.Add(rbf.Key{Shard: 1}, 0x00000001, 0x00000002, 0x00010003, 0x00030004); err != nil {
		t.Fatal(err)
	}
...
func TestCursor_RLETesting(t *testing.T) {
	...
	_, err := tx.AddRoaring(rbf.Key{Shard: 1}, rb)
	if err != nil {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			changeCount, err := tx.Add(rbf.Key{Shard: 1}, tt.args...)
...
func TestCursor_BitmapBitN(t *testing.T) {
	...
	err := tx.PutContainer(rbf.Key{Shard: 1}, 0, ct)
	if err != nil {
	...
	v, err := tx.Container(rbf.Key{Shard: 1}, 0)
	if err != nil {
...
func TestCursor_RLEConversion(t *testing.T) {
	...
	for x := uint64(65408); x < 65536; x++ {
		_, err = tx.Add(rbf.Key{Shard: 1}, x)
		if err != nil {
			panic(err)
		}
		//vv("on i=%v, x = %v", i, x)
...
func (e *EasyWalker) Visitor(pgno uint32, records []rbf.Record) {
	for _, record := range records {
		e.VisitRoot(record.Page, record.Key())
		rbf.WalkPage(e.tx, record.Page, e)
	}
}
func (e *EasyWalker) VisitRoot(pgno uint32, name rbf.Key) {
...
func TestCursor_SplitBranchCells(t *testing.T) {
	...
	for i := 0; i < numContainers; i++ { //need to calculate how many will force a split
		b := rb(uint64(i))
		if _, err := tx.AddRoaring(rbf.Key{Shard: 1}, b); err != nil {
			panic(err)
		}
	}
	...
	if _, err := tx.AddRoaring(rbf.Key{Shard: 1}, rb(uint64(numContainers))); err != nil {
		panic(err)
	}
	...
	//
	c, _ := tx.Cursor(rbf.Key{Shard: 1}) //added just for dot code coverage
	c.Dump("test.dump")
	os.Remove("test.dump")
}
...
func TestCursor_RemoveCells(t *testing.T) {
	...
	cur, _ := tx.Cursor(rbf.Key{Shard: 1})
	rb := func(key uint64) *roaring.Bitmap {
		bm := roaring.NewBitmap()
		bits := make([]uint64, rbf.BitmapN)
		n := 0
...
func TestCursor_OneBitmap(t *testing.T) {
	...
	cur, err := tx.Cursor(rbf.Key{Shard: 1})
	if err != nil {
		panic(err)
	}
	if err := cur.First(); err != nil {
...
func TestCursor_GenerateAll(t *testing.T) {
	...
	if _, err := tx.AddRoaring(rbf.Key{Shard: 1}, ar); err != nil {
		panic(err)
	}
	rb := func() *roaring.Bitmap {
		bm := roaring.NewBitmap()
	...
	if _, err := tx.AddRoaring(rbf.Key{Shard: 1}, rb); err != nil {
		panic(err)
	}
	...
	if _, err := tx.AddRoaring(rbf.Key{Shard: 1}, bb); err != nil {
		panic(err)
	}
	if err := tx.CreateBitmap(rbf.Key{Shard: 4}); err != nil {
		t.Fatal(err)
	}
	if _, err := tx.AddRoaring(rbf.Key{Shard: 4}, bb); err != nil {
		panic(err)
	}
}
...
func TestCursor_PutContainer(t *testing.T) {
	t.Run("BitmapToArray", func(t *testing.T) {
		...
		if err := tx.PutContainer(rbf.Key{Shard: 1}, 0, roaring.NewContainerBitmap(-1, bmData)); err != nil {
			t.Fatal(err)
		}
		if err := tx.PutContainer(rbf.Key{Shard: 1}, 0, roaring.NewContainerArray([]uint16{0})); err != nil {
			t.Fatal(err)
		}
	...
	t.Run("BitmapToBitmap", func(t *testing.T) {
		...
		if err := tx.PutContainer(rbf.Key{Shard: 1}, 0, roaring.NewContainerBitmap(-1, data0)); err != nil {
			t.Fatal(err)
		}
		...
		if err := tx.PutContainer(rbf.Key{Shard: 1}, 0, roaring.NewContainerBitmap(-1, data1)); err != nil {
			t.Fatal(err)
		}
