...
func TestDB_WAL(t *testing.T) {
	t.Run("ErrTxTooLarge", func(t *testing.T) {
		...
		if err := tx.CreateBitmap(rbf.Key{Column: 1}); err != nil {
			t.Fatal(err)
		} else if err := tx.CreateBitmap(rbf.Key{Column: 2}); err != rbf.ErrTxTooLarge {
	...
	t.Run("ErrTxTooLargeWithBitmap", func(t *testing.T) {
		...
		for i := uint64(0); i < rbf.ArrayMaxSize; i++ {
			if _, err := tx.Add(rbf.Key{Column: 1}, i); err != nil {
				t.Fatal(err)
			}
		}
		...
		if _, err := tx.Add(rbf.Key{Column: 1}, rbf.ArrayMaxSize); err == nil || !errors.Is(err, rbf.ErrTxTooLarge) {
			t.Fatalf("unexpected error: %#v", err)
		}
	})
		...
		for i := 0; i < 100; i++ {
			func() {
				...
				if err := tx.CreateBitmapIfNotExists(rbf.Key{Column: 1}); err != nil {
					t.Fatal(err)
				} else if _, err := tx.Add(rbf.Key{Column: 1}, uint64(i)); err != nil {
					t.Fatal(err)
			...
			g.Go(func() error {
				...
				for {
					...
					if err := func() error {
						...
						_, err = tx.Container(rbf.Key{Column: 1}, 0)
						if err != nil {
...
func TestDB_Recovery(t *testing.T) {
	...
	t.Run("TruncPartialWALBitmap", func(t *testing.T) {
		...
		if tx, err := db.Begin(true); err != nil {
			...
			t.Fatal(err)
		} else if _, err := tx.Add(rbf.Key{Column: 1}, a...); err != nil {
			t.Fatal(err)
		} else if err := tx.Commit(); err != nil {
			t.Fatal(err)
		}
		...
		if err != nil {
			t.Fatal(err)
		} else if _, err := tx0.Add(rbf.Key{Column: 1}, uint64(len(a))); err != nil {
			t.Fatal(err)
		}
		...
		if exists, err := tx.Contains(rbf.Key{Column: 1}, uint64(len(a))); exists || err != nil {
			t.Fatalf("Contains()=<%v,%#v>", exists, err)
		} else if exists, err := tx.Contains(rbf.Key{Column: 1}, uint64(len(a)-1)); !exists || err != nil {
			t.Fatalf("Contains()=<%v,%#v>", exists, err)
		}
		tx.Rollback()
	})
...
func TestDB_HasData(t *testing.T) {
	...
	if tx, err := db.Begin(true); err != nil {
		t.Fatal(err)
	} else if _, err := tx.Add(rbf.Key{Column: 1}, rand.Uint64()); err != nil {
		t.Fatal(err)
	} else if err := tx.Commit(); err != nil {
		t.Fatal(err)
	}
...
func TestDB_MultiTx(t *testing.T) {
		...
		g.Go(func() error {
			for {
				if ctx.Err() != nil {
					...
					for i := 0; i < n; i++ {
						v := rand.Intn(1 << 20)
						if _, err := tx.Contains(rbf.Key{Column: 1}, uint64(v)); err != nil {
							return err
						}
					}
					return nil
	...
	for i := 0; i < 100; i++ {
		func() {
			...
			for j := 0; j < n; j++ {
				v := rand.Intn(1 << 20)
				if _, err := tx.Add(rbf.Key{Column: 1}, uint64(v)); err != nil {
					t.Fatal(err)
				}
		...
		func() {
			...
			for j := 0; j < times; j++ {
				...
				if j&7 == 0 {
					...
					if _, err := tx.Remove(rbf.Key{Column: 1}, uint64(v)); err != nil {
						b.Fatal(err)
					}
				} else {
					if _, err := tx.Add(rbf.Key{Column: 1}, uint64(v)); err != nil {
...
func TestDB_Backup(t *testing.T) {
	...
	_, err = tx.Add(rbf.Key{Column: 1}, 1, 2, 3)
	if err != nil {
		t.Fatal(err)
	}
	err = tx.Commit()
	...
	r, err := tx.RoaringBitmap(rbf.Key{Column: 1})
	if err != nil {
		t.Fatal(err)
	}
	want := []uint64{1, 2, 3}
